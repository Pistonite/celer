...
static CACHE: Lazy<Mutex<PreparedContextCache>> = Lazy::new(|| Mutex::new(PreparedContextCache::new()));
pub async fn get_context(owner: &str, repo: &str, path: Option<&str>, reference: &str) -> PrepResult<RefCounted<PreparedContext<ServerResourceLoader>>> {
...
impl PreparedContextCache {
    ...
    pub async fn get(&mut self, owner: &str, repo: &str, path: Option<&str>, reference: &str) -> PrepResult<RefCounted<PreparedContext<ServerResourceLoader>>> {
        ...
        let mut builder = super::new_context_builder(owner, repo, Some(reference)).with_route_built();
