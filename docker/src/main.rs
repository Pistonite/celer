...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let token = std::env::var("GITHUB_TOKEN")
        .expect("GITHUB_TOKEN not set! Need a PAT with repo read permission");
    ...
    let res = client
        .get("https://api.github.com/repos/Pistonite/celer/actions/artifacts")
    ...
    let artifacts = value
        ...
        .get("artifacts")
        .ok_or("expected `artifacts` in object")?
        .as_array()
        .ok_or("expected `artifacts` to be array")?
        .iter()
        .filter(|artifact| match filter_artifact(artifact, &branch, &rev) {
            Ok(x) => x,
            Err(e) => {
                eprintln!("Error: {}", e);
                false
...
fn filter_artifact(artifact: &Value, branch: &str, rev: &str) -> Result<bool, &'static str> {
    let workflow_run = artifact
        .as_object()
        .ok_or("expected artifact object")?
        .get("workflow_run")
        .ok_or("expected `workflow_run` in object")?
        .as_object()
        .ok_or("expected `workflow_run` to be object")?;

    let head_branch = workflow_run
        .get("head_branch")
        .ok_or("expected `head_branch` in object")?
        .as_str()
        .ok_or("expected `head_branch` to be string")?;
    ...
    let head_sha = workflow_run
        .get("head_sha")
        .ok_or("expected `head_sha` in object")?
        .as_str()
        .ok_or("expected `head_sha` to be string")?;
...
async fn download_artifact(
    ...
    let name = artifact
        .get("name")
        .ok_or("expected `name` in object")?
        .as_str()
        .ok_or("expected `name` to be string")?;

    println!("Downloading artifact {}", name);

    let download_url = artifact
        .get("archive_download_url")
        .ok_or("expected `archive_download_url` in object")?
        .as_str()
        .ok_or("expected `archive_download_url` to be string")?;
