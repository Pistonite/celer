...
export const SwitchToolbarLocation: ToolbarControl = {
    ToolbarButton: React.forwardRef<HTMLButtonElement>((_, ref) => {
        ...
        return (
            <SwitchToolbarLocationInternal layout={layout} locations={availableToolbarLocations}>
                <Tooltip
                    content={
                        isDefaultLayout
                    ...
                    relationship="label"
                >
                    <ToolbarButton disabled={isDefaultLayout} icon={<Window20Regular />} ref={ref} />
                </Tooltip>
    ...
    MenuItem: () => {
        ...
        return (
            <SwitchToolbarLocationInternal layout={layout} locations={availableToolbarLocations}>
                <MenuItem disabled={isDefaultLayout} icon={<Window20Regular />}>Toolbar Location</MenuItem>
            </SwitchToolbarLocationInternal>
        );
    }
};
...
const SwitchToolbarLocationInternal: React.FC<SwitchToolbarLocationInternalProps> = ({
    children, 
    layout, 
    locations 
}) => {
    ...
    const onChangeToolbarMenuCheckedItems: OnMenuCheckedValueChangeFunction = (_, { name, checkedItems }) => {
                ...
                <MenuList>
                    {
                        locations.map((location) => (
                            <MenuItemRadio key={location} name={ToolbarLocationRadioName} value={location}>
