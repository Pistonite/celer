...
export const ErrorBar: React.FC<PropsWithChildren<ErrorBarProps>> = ({children, title}) => {
    if (!children) {
        return null;
    }
    return (
...
export const FormattedError: React.FC<{error: unknown}> = ({error}) => {
    ...
    const lines = error.split("\n").map((line, i) => {
        if (!line) {
            return <div className={ErrorBarClass.className} key={i}>&nbsp;</div>;
        }
        return <div className={ErrorBarClass.className} key={i}>{line || ""}</div>;
    });
