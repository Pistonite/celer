...
import { AnimalCat20Regular, ArrowDownload20Regular, Box20Regular, Code20Regular, Document20Regular, DocumentChevronDouble20Regular, FolderZip20Regular, Image20Regular, Video20Regular } from "@fluentui/react-icons";
...
const useExportControl = () => {
    ...
    const enabled = document !== undefined && exportMetadata !== undefined && exportMetadata.length > 0;
    return {
        enabled,
        exportMetadata,
...
const getTooltip = (document: ExecDoc | undefined, exportMetadata: ExportMetadata[] | undefined) => {
    if (!document) {
...
const ExportInternal: React.FC<ExportInternalProps> = ({children, exportMetadata}) => {
    return (
        <Menu>
...
const ExportButton: React.FC<{metadata: ExportMetadata}> = ({metadata}) => {
    const text = metadata.extension ? `${metadata.name} (*.${metadata.extension})` : metadata.name;
    return (
        <Tooltip 
...
const ExportIconComponent: React.FC<{name: ExportIcon | undefined}> = ({name}) => {
    switch (name) {
        case "archive": return <FolderZip20Regular />;
        case "binary": return <Box20Regular />;
        case "cat": return <AnimalCat20Regular />;
        case "code": return <Code20Regular />;
        case "data": return <DocumentChevronDouble20Regular />;
        case "file": return <Document20Regular />;
        case "image": return <Image20Regular />;
        case "video": return <Video20Regular />;
        default: 
