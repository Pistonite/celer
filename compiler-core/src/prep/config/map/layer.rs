...
impl<'a> PreparedConfig<'a> {
    pub fn parse_map_layer(&self, value: Value, layer_index: usize) -> PrepResult<MapLayer> {
        ...
        let template_url = if template_url.is_array() || template_url.is_object() {
            Err(self.err_invalid_map_layer_property_type(
                prop::TEMPLATE_URL,
                layer_index,
                "string",
            ))?
        ...
        let size = parse_array_with_2_elements(size).ok_or_else(|| {
            self.err_invalid_map_layer_property_type(
                prop::SIZE,
                layer_index,
                "array of 2 non-negative integers",
            )
        })?;
    ...
    fn err_invalid_map_layer_property_type(
        &self,
        prop: &str,
        layer_index: usize,
        ...
        PrepError::InvalidConfigPropertyType(
            self.trace.clone(),
            format!("{}.{}[{}].{}", prop::MAP, prop::LAYERS, layer_index, prop,).into(),
            expected_type.into(),
        )
    }
}
