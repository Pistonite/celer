...
pub mod pt {
  ...
  #[derive(Debug)] pub enum Block<'p> { 
    ...
    NonDollar(Box<ParseHook<String, pt::NonDollar<'p>>>),
  }
...
regen::impl_union!(from_ast_internal, NonDollar, {
  Text,
  Number,
  Symbol,
});
impl<'p> pt::NonDollar<'p> {
  #[inline] #[allow(clippy::unnecessary_mut_passed)] fn from_ast(ast: &'p ast::NonDollar, ctx: &mut Ctx) -> ParseHook<String, pt::NonDollar<'p>> {
    let mut pt = Self::from_ast_internal(ast, ctx);
    ParseHook { val: parse_non_dollar(&mut pt, ctx), pt }
  }
}
impl ast::Variable {
  pub fn parse(ts: &mut TokenStream<Tok>) -> Option<Self> {
