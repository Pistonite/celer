...
export class FsImplFe implements FsFileSystemUninit, FsFileSystem, FsFileSystemInternal {
    ...
    public async listDir(path: string): Promise<FsResult<string[]>> {
        ...
        const entries = await tryAsync(() => new Promise<FileSystemEntry[]>((resolve, reject) => {
    ...
    public async read(path: string): Promise<FsResult<File>> {
        ...
        const file = await tryAsync(() => new Promise<File>((resolve, reject) => {
    ...
    private async resolveDir(path: string): Promise<FsResult<FileSystemDirectoryEntry>> {
        ...
        const entry = await tryAsync(() => new Promise<FileSystemEntry>((resolve, reject) => {
